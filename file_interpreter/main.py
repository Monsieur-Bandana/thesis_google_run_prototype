...
def main():
    ...
    for file in list_of_jsons:
        ...
        if "classification" in filename:
            download_file_from_bucket(
                bucket_name, file, f"{main_folder}/temp/{filename}"
            )
    ...
    for dir in dirs:
        ...
        for entity in entities:
            ...
            for pdf_name in fitting_pdfs:
                if not check_if_file_got_already_interpreted(
                    f"{main_folder}/temp/{dir}-{class_name}.txt",
                    f"""{dir}/{pdf_name.split("/")[1]}""",
                ):
                    local_pdf_path = f"""{main_folder}/temp/{pdf_name.split("/")[1]}"""
                    download_file_from_bucket(
                        bucket_name, f"raw_pdf_files/{pdf_name}", local_pdf_path
                    )
                    summary = summarize_pdf_content(
                        local_pdf_path, class_name, class_description, related_terms
                    )
            ...
            if summaries:
                ...
                upload_file(
                    bucket_name, file_in_question, f"summaries/{dir}-{class_name}.txt"
                )
